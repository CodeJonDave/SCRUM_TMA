...
CREATE TABLE IF NOT EXISTS sprints
 ...
 status VARCHAR(30) CHECK (status IN ('PLANNED',
                                      'ACTIVE',
                                      'COMPLETED',
                                      'CANCELED')) DEFAULT 'PLANNED', -- Status values:
...
CREATE TABLE IF NOT EXISTS sprints_olap (log_id SERIAL PRIMARY KEY, -- Unique identifier for each log
 sprint_id INTEGER, -- The ID of the sprint item that changed
 operation_type VARCHAR(10) NOT NULL, -- The type of operation performed
...
CREATE OR REPLACE FUNCTION log_sprints_changes() RETURNS TRIGGER AS $$
BEGIN
    ...
    ELSIF (TG_OP ='DELETE') THEN
        INSERT INTO sprints_olap (sprint_id, operation_type, data)
        VALUES (OLD.sprint_id, 'DELETE', row_to_json(OLD));
        RETURN OLD;
    END IF;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_sprints_changes AFTER
INSERT
OR
UPDATE
OR
