...
CREATE TABLE IF NOT EXISTS sprint_backlog (
    ...
    assigned_team_member UUID NOT NULL, -- Team member responsible for the backlog item
    estimated_time TIME NOT NULL, -- Estimated time for the backlog item to complete
    status VARCHAR(30) CHECK (status IN ('To Do', 'In Progress', 'Blocked', 'In Review', 'Done', 'Carried Over', 'Removed')) DEFAULT 'To Do', -- Status
    ...
    UNIQUE (sprint_id, product_backlog_item_id),
    -- Foreign Key constraints
    CONSTRAINT fk_product_backlog_item_id FOREIGN KEY (product_backlog_item_id) REFERENCES product_backlog (backlog_item_id) ON DELETE CASCADE,
    CONSTRAINT fk_sprint_id FOREIGN KEY (sprint_id) REFERENCES sprints (sprint_id) ON DELETE CASCADE,
    CONSTRAINT fk_assigned_team_member FOREIGN KEY (assigned_team_member) REFERENCES users (user_id)
);
...
-- 3. Create the Sprint Backlog OLAP Table
CREATE TABLE IF NOT EXISTS sprint_backlog_olap (
    ...
    operation_type VARCHAR(10) NOT NULL, -- The type of operation (INSERT, UPDATE, DELETE)
    data JSONB NOT NULL, -- Full sprint_backlog data in JSONB format
...
-- 5. Create a trigger to log changes on INSERT, UPDATE, or DELETE
CREATE TRIGGER trg_sprint_backlog_changes
AFTER INSERT OR UPDATE OR DELETE ON sprint_backlog
FOR EACH ROW EXECUTE FUNCTION log_sprint_backlog_changes();
